...
#define BUFFERSIZE 36  // Number of bits+4
  ...
  #else
  #define WORKSPACESIZE 22000          /* Cells (8*bytes) */
  #endif
  #define LITTLEFS
  #include <LittleFS.h>
#else
...
enum builtins: builtin_t { NIL, TEE, NOTHING, OPTIONAL, FEATURES, INITIALELEMENT, ELEMENTTYPE, TEST, BIT, AMPREST, LAMBDA, LET, LETSTAR,
CLOSURE, PSTAR, QUOTE, DEFUN, DEFVAR, EQ, CAR, FIRST, CDR, REST, NTH, AREF, CHAR, STRINGFN, PINMODE, DIGITALWRITE,
...
#endif
#if defined(gfxsupport)
inline void gfxwrite (char c) { M5Cardputer.Display.write(c); }
#endif
...
object *fn_drawpixel (object *args, object *env) {
  ...
  M5Cardputer.Display.drawPixel(checkinteger(first(args)), checkinteger(second(args)), colour);
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_drawrect (object *args, object *env) {
  ...
  for (int i=0; i<4; i++) { params[i] = checkinteger(car(args)); args = cdr(args); }
  if (args != NULL) colour = checkinteger(car(args));
  M5Cardputer.Display.drawRect(params[0], params[1], params[2], params[3], colour);
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_fillrect (object *args, object *env) {
  ...
  for (int i=0; i<4; i++) { params[i] = checkinteger(car(args)); args = cdr(args); }
  if (args != NULL) colour = checkinteger(car(args));
  M5Cardputer.Display.fillRect(params[0], params[1], params[2], params[3], colour);
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_drawcircle (object *args, object *env) {
  (void) env;
  #if defined(gfxsupport)
  uint16_t params[3], colour = WHITE;
  for (int i=0; i<3; i++) { params[i] = checkinteger(car(args)); args = cdr(args); }
  if (args != NULL) colour = checkinteger(car(args));
  M5Cardputer.Display.drawCircle(params[0], params[1], params[2], colour);
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_fillcircle (object *args, object *env) {
  ...
  for (int i=0; i<3; i++) { params[i] = checkinteger(car(args)); args = cdr(args); }
  if (args != NULL) colour = checkinteger(car(args));
  M5Cardputer.Display.fillCircle(params[0], params[1], params[2], colour);
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_drawroundrect (object *args, object *env) {
  (void) env;
  #if defined(gfxsupport)
  uint16_t params[5], colour = WHITE;
  for (int i=0; i<5; i++) { params[i] = checkinteger(car(args)); args = cdr(args); }
  if (args != NULL) colour = checkinteger(car(args));
  M5Cardputer.Display.drawRoundRect(params[0], params[1], params[2], params[3], params[4], colour);
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_fillroundrect (object *args, object *env) {
  ...
  for (int i=0; i<5; i++) { params[i] = checkinteger(car(args)); args = cdr(args); }
  if (args != NULL) colour = checkinteger(car(args));
  M5Cardputer.Display.fillRoundRect(params[0], params[1], params[2], params[3], params[4], colour);
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_drawtriangle (object *args, object *env) {
  (void) env;
  #if defined(gfxsupport)
  uint16_t params[6], colour = WHITE;
  for (int i=0; i<6; i++) { params[i] = checkinteger(car(args)); args = cdr(args); }
  if (args != NULL) colour = checkinteger(car(args));
  M5Cardputer.Display.drawTriangle(params[0], params[1], params[2], params[3], params[4], params[5], colour);
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_filltriangle (object *args, object *env) {
  ...
  for (int i=0; i<6; i++) { params[i] = checkinteger(car(args)); args = cdr(args); }
  if (args != NULL) colour = checkinteger(car(args));
  M5Cardputer.Display.fillTriangle(params[0], params[1], params[2], params[3], params[4], params[5], colour);
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_drawchar (object *args, object *env) {
  (void) env;
  #if defined(gfxsupport)
  uint16_t colour = WHITE, bg = BLACK, size = 1;
  ...
  M5Cardputer.Display.drawChar(checkinteger(first(args)), checkinteger(second(args)), checkchar(third(args)),
    colour, bg, size);
  #else
  (void) args;
  #endif
...
object *fn_setcursor (object *args, object *env) {
  (void) env;
  #if defined(gfxsupport)
  M5Cardputer.Display.setCursor(checkinteger(first(args)), checkinteger(second(args)));
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_settextcolor (object *args, object *env) {
  (void) env;
  #if defined(gfxsupport)
  if (cdr(args) != NULL) M5Cardputer.Display.setTextColor(checkinteger(first(args)), checkinteger(second(args)));
  else M5Cardputer.Display.setTextColor(checkinteger(first(args)));
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_settextsize (object *args, object *env) {
  (void) env;
  #if defined(gfxsupport)
  M5Cardputer.Display.setTextSize(checkinteger(first(args)));
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_settextwrap (object *args, object *env) {
  (void) env;
  #if defined(gfxsupport)
  M5Cardputer.Display.setTextWrap(first(args) != NULL);
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_fillscreen (object *args, object *env) {
  ...
  M5Cardputer.Display.fillScreen(colour);
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_setrotation (object *args, object *env) {
  (void) env;
  #if defined(gfxsupport)
  M5Cardputer.Display.setRotation(checkinteger(first(args)));
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_invertdisplay (object *args, object *env) {
  (void) env;
  #if defined(gfxsupport)
  M5Cardputer.Display.invertDisplay(first(args) != NULL);
  #else
  (void) args;
  #endif
  return nil;
...
// M5 cardputer terminal and keyboard support
...
void ScrollDisplay () {
  ...
  for (uint8_t x = 0; x < Columns; x++) {
    ...
    for (uint8_t y = 0; y < Lines-1; y++) {
      ...
      if (c != c2) {
        if (c2 & 0x80) {
          M5Cardputer.Display.drawChar(x*CharWidth, y*Leading, c2 & 0x7f, BLACK, GREEN, 1);
        } else {
          M5Cardputer.Display.drawChar(x*CharWidth, y*Leading, c2 & 0x7f, BLACK, WHITE, 1);
        }
        c = c2;
      }
    }
...
void Display (char c) {
  ...
  // Control characters
    ...
    M5Cardputer.Display.fillScreen(BLACK); line = 0; column = 0; Scroll = 0;
